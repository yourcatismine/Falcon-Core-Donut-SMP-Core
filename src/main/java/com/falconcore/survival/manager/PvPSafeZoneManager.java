 ...
 * Uses database storage for persistence, or YAML flatfile when database is disabled
 */
public class PvPSafeZoneManager {
    ...
    public PvPSafeZoneManager(Falcon plugin, DatabaseManager databaseManager) {
        this.plugin = plugin;
    ...
    /**
     * Load all safe zones from the database or YAML
     */
    public void loadZones() {
        if (isFlatfileMode()) {
            safeZones.clear();
            safeZones.addAll(getYamlStorage().loadAllPvpSafeZones());
            plugin.getLogger().info("Loaded " + safeZones.size() + " PvP safe zones from YAML.");
            return;
        }
    ...
    public boolean addZone(String name, String world, double minX, double minY, double minZ, 
        ...
        if (isFlatfileMode()) {
            ...
            getYamlStorage().savePvpSafeZone(name, world, minX, minY, minZ, maxX, maxY, maxZ, createdBy, createdAt);
            return true;
        }
    ...
    public boolean removeZone(String zoneName) {
        if (isFlatfileMode()) {
            boolean removed = getYamlStorage().deletePvpSafeZone(zoneName);
            if (removed) {
