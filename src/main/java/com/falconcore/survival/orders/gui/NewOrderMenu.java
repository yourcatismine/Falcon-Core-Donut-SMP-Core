...
public class NewOrderMenu implements InventoryHolder, MenuOwner {
    ...
    public void open() {
        if (this.p.hasMetadata("falconorder.tmpChosenStack")) {
            List<MetadataValue> vals = this.p.getMetadata("falconorder.tmpChosenStack");
            if (!vals.isEmpty()) {
                ItemStack stack = (ItemStack) vals.get(0).value();
                this.selected = ItemKey.fromStack(stack);
                this.p.removeMetadata("falconorder.tmpChosenStack", this.module.getPlugin());
            }
        }
        if (this.p.hasMetadata("falconorder.tmpAmount")) {
            List<MetadataValue> vals = this.p.getMetadata("falconorder.tmpAmount");
            if (!vals.isEmpty()) {
                this.amount = vals.get(0).asInt();
                this.p.removeMetadata("falconorder.tmpAmount", this.module.getPlugin());
            }
        }
        if (this.p.hasMetadata("falconorder.tmpPrice")) {
            List<MetadataValue> vals = this.p.getMetadata("falconorder.tmpPrice");
            if (!vals.isEmpty()) {
                this.price = vals.get(0).asDouble();
                this.p.removeMetadata("falconorder.tmpPrice", this.module.getPlugin());
            }
        }
        ...
        if (!this.p.hasMetadata("falconorder.startTime")) {
            this.p.setMetadata("falconorder.startTime",
    ...
    public void onClick(InventoryClickEvent e) {
        ...
        if (slot == 12) {
            ...
            new SelectItemMenu(this.module, this.p, (item) -> {
                if (item != null && this.module.ench().hasOptionsFor(item.material)) {
                    ...
                    this.p.setMetadata("falconorder.tmpAmount",
                            new FixedMetadataValue(this.module.getPlugin(), this.amount));
                    this.p.setMetadata("falconorder.tmpPrice",
        ...
        if (slot == 16) {
            ...
            this.p.removeMetadata("falconorder.startTime", this.module.getPlugin());
    ...
    public void onClose(InventoryCloseEvent e) {
        ...
        if (this.p.hasMetadata("falconorder-sign-input")) {
            return;
        }
