...
public class AdminOrderLootMenu implements InventoryHolder, MenuOwner {
    private static final String META_SUPPRESS_RETURN = "falconorder.suppressReturn";
            ...
            synchronized (order.storage) {
                ...
                if (itemIndex < order.storage.size()) {
                    ...
                    if (targetItem != null && targetItem.getType() != Material.AIR) {
                        ...
                        com.falconcore.survival.tools.ToolsManager.getInstance().resumeOrdersTimers(targetItem);
        ...
        if (slot == 48) {
            synchronized (order.storage) {
                ...
                for (int i = end - 1; i >= start; i--) {
                    ...
                    if (item != null && item.getType() != Material.AIR) {
                        com.falconcore.survival.tools.ToolsManager.getInstance().resumeOrdersTimers(item);
                        toDrop.add(item);
                        droppedCount++;
                    }
                }
    ...
    public void onClose(InventoryCloseEvent e) {
        ...
        com.falconcore.survival.orders.util.TaskUtil.runEntityLater(this.module.getPlugin(), this.admin,
