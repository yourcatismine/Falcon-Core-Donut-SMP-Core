...
public class ConfirmDeliveryMenu
    ...
    public void open() {
        ...
        if (!this.p.hasMetadata("falconorder.deliveryStartTime")) {
            this.p.setMetadata("falconorder.deliveryStartTime",
    ...
    public void onClick(InventoryClickEvent e) {
        ...
        if (slot == 11) {
            ...
            for (ItemStack shulker : this.originalShulkers) {
                com.falconcore.survival.tools.ToolsManager.getInstance().resumeOrdersTimers(shulker);
                this.giveBackOrDrop(shulker);
            }

            for (ItemStack directItem : this.acceptedDirect) {
                com.falconcore.survival.tools.ToolsManager.getInstance().resumeOrdersTimers(directItem);
